    ...
    if (allSsa) {
      ...
      for (let phaseIdx = 0; phaseIdx < phases.length; phaseIdx++) {
        ...
        while (t < phaseTEnd) {
          ...
          // OPT 3: Inlined PRNG calls (Mulberry32) avoiding function call overhead and divisions
          ...
          // mathematically identical to (1 / aTot) * Math.log(1 / r1), but avoids 1 / r1 division!
          ...
          // OPT 7: adaptive selection (linear scan for small R, Fenwick for large R) inlined to save call overhead
          ...
          if (compiledSSAEventUpdater) {
            ...
            for (let d = 0; d < deps.length; d++) {
              ...
              if (useFenwick) {
                let fi = jrxn + 1;
                while (fi <= numReactions) {
                  fenwickTree[fi] += delta;
                  fi += fi & -fi;
                }
              }
