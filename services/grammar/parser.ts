...
const PATTERNS = {
  ...
  DEFINITION:
    /^(?:Define|Create|Declare|Add)\s+(?:molecule|protein|species|receptor|kinase|enzyme)?\s*(\w+)(?:\s+(?:with|having)\s+(?:sites?|domains?)\s+(.+))?/i,

  // Compartment definition
  COMPARTMENT:
    /^(?:Define|Create)\s+compartment\s+(\w+)(?:\s+(?:with\s+)?(?:volume|size)\s+([0-9.e-]+))?(?:\s+(?:dimension|dim)\s+(\d))?/i,

  // Observables
  OBSERVABLE:
    /^(?:Observe|Track|Monitor|Measure|Watch)\s+(.+?)(?:\s+(?:as|named)\s+(\w+))?$/i,

  // Initialization - very flexible
  INITIALIZATION:
    /^(?:Start|Begin|Initialize|Set)\s+(?:with\s+)?(\d+|[0-9.e]+)\s+(?:of\s+|copies\s+of\s+|molecules?\s+of\s+)?(.+)/i,

  // Simulation
  SIMULATION:
    /^(?:Simulate|Run|Execute)\s+(?:for\s+)?([0-9.e]+)\s*(?:s|sec|seconds?)?(?:\s+(?:with|using)\s+([0-9]+)\s+(?:steps?|points?))?/i,
...
function getSuggestion(text: string): ParseSuggestion {
  ...
  if (/\bphos|\bkinase/.test(lowerText)) {
    return {
      ...
      examples: [
        ...
        "Kinase phosphorylates Substrate at site Y",
      ],
    };
  }
  ...
  if (/\bdephos|\bphosphatase/.test(lowerText)) {
    return {
      ...
      examples: [
        "SHP1 dephosphorylates TCR",
        "Phosphatase removes phosphate from Target",
      ],
    };
  }

  if (
    /\bsynth|\bproduc|\bcreate|\bexpress|\btranscrib|\btranslat/.test(lowerText)
  ) {
    return {
  ...
  if (/\bactivat|\bturn\s*on|\benabl|\bstimulat|\bpromot/.test(lowerText)) {
    return {
      ...
      examples: [
        "Kinase activates Substrate",
        "Ligand stimulates Receptor",
        "Enzyme enables Pathway",
      ],
    };
  }

  if (
    /\binhib|\bblock|\bsuppress|\brepress|\bprevent|\binactivat/.test(lowerText)
  ) {
    return {
      message: "Looks like inhibition. Try:",
      examples: [
        "Inhibitor blocks Enzyme",
        "Drug suppresses Receptor",
  ...
  if (/\bcleav|\bcut|\bsplit|\bprocess|\bproteo/.test(lowerText)) {
    return {
      ...
      examples: [
        ...
        "Protease cuts Protein at site S",
      ],
    };
  }
  ...
  if (/\bubiquit/.test(lowerText)) {
    return {
      ...
      examples: [
        ...
        "Ligase adds ubiquitin to Substrate",
      ],
    };
  }
  ...
  if (/\bmethyl/.test(lowerText)) {
    return {
      ...
      examples: [
        "Methyltransferase methylates Histone",
        "Enzyme adds methyl to Target",
      ],
    };
  }
  ...
  if (/\bdefin|\bcreate|\bdeclar|\badd/.test(lowerText)) {
    return {
      ...
      examples: [
        ...
        "Create protein Receptor with sites a, b~active~inactive",
      ],
    };
  }
  ...
  if (/\bstart|\bbegin|\binitial|\bset/.test(lowerText)) {
    return {
      ...
      examples: [
        ...
        "Initialize 50 TCR molecules",
        "Begin with 1000 Ligand",
      ],
    };
  }
  ...
  if (/\bobserv|\btrack|\bmonitor|\bmeasur|\bwatch/.test(lowerText)) {
    return {
      ...
      examples: [
        "Observe Lck",
        "Track phosphorylated TCR as pTCR",
        "Monitor total Receptor",
      ],
    };
  }
  ...
  if (/\bcompart/.test(lowerText)) {
    return {
      ...
      examples: [
        "Define compartment cytoplasm with volume 1",
        "Create compartment membrane dim 2",
      ],
    };
  }
...
export class BioParser {
  static parseDocument(text: string): BioSentence[] {
    ...
    return lines
      .map((line) => this.parseSentence(line.trim()))
      .filter((s) => s !== null) as BioSentence[];
  }

  static parseSentence(text: string): BioSentence {
    if (!text || text.trim() === "")
      return { id: uuidv4(), text, type: "COMMENT", isValid: true };
    if (PATTERNS.COMMENT.test(text))
      return { id: uuidv4(), text, type: "COMMENT", isValid: true };
  ...
  private static tryParseInteraction(text: string): InteractionSentence | null {
    ...
    const extractEntities = (
      ...
      return {
        ...
        rate:
          match[options.selfAction ? 2 : 3] || options.defaultRate || "k_fwd",
        reverseRate: options.isBidirectional
          ? match[options.selfAction ? 3 : 4] ||
            options.defaultReverseRate ||
            "k_rev"
          : undefined,
        site: options.extractSite
          ? match[options.selfAction ? 2 : 3]
          : undefined,
    ...
    const bindPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(BINDING_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+)(?:[,\\s]+([0-9.e-]+))?)?`,
      "i",
    );
    ...
    // PHOSPHORYLATION
    const phosPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(PHOSPHORYLATION_VERBS)})\\s+(\\w+)(?:\\s+(?:at|on)\\s+(\\w+))?(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    ...
    // DEPHOSPHORYLATION
    const dephosPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(DEPHOSPHORYLATION_VERBS)})\\s+(\\w+)(?:\\s+(?:at|on)\\s+(\\w+))?(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    ...
    const synthPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(SYNTHESIS_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    const synthResult = extractEntities(synthPattern, "synthesizes", {
      defaultRate: "k_syn",
    });
    ...
    const degPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(DEGRADATION_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    const degResult = extractEntities(degPattern, "degrades", {
      defaultRate: "k_deg",
    });
    if (degResult) return degResult;

    // Alternative degradation: "X is degraded" / "X degrades"
    const passiveDegPattern =
      /^(\w+)\s+(?:is\s+)?(?:degraded|destroyed|eliminated)(?:\s+(?:with\s+)?(?:rate|k)\s+([0-9.e-]+))?/i;
    ...
    const dimerSelfPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(DIMERIZATION_VERBS)})(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+)(?:[,\\s]+([0-9.e-]+))?)?$`,
      "i",
    );
    ...
    // Heterodimerization
    const dimerWithPattern =
      /^(\w+)\s+(?:dimerizes|forms\s+dimer|heterodimerizes)\s+with\s+(\w+)(?:\s+(?:with\s+)?(?:rate|k)\s+([0-9.e-]+)(?:[,\s]+([0-9.e-]+))?)?/i;
    ...
    // TRANSLOCATION
    const translocPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(TRANSLOCATION_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    ...
    const actPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(ACTIVATION_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    const actResult = extractEntities(actPattern, "activates", {
      defaultRate: "k_act",
    });
    if (actResult) return actResult;

    // INHIBITION
    const inhibPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(INHIBITION_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    const inhibResult = extractEntities(inhibPattern, "inhibits", {
      defaultRate: "k_inhib",
    });
    if (inhibResult) return inhibResult;

    // CLEAVAGE
    const cleavePattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(CLEAVAGE_VERBS)})\\s+(\\w+)(?:\\s+(?:at|into)\\s+(\\w+))?(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    ...
    // UBIQUITINATION
    const ubiqPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(UBIQUITINATION_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    const ubiqResult = extractEntities(ubiqPattern, "ubiquitinates", {
      defaultRate: "k_ubiq",
    });
    if (ubiqResult) return ubiqResult;

    // DEUBIQUITINATION
    const deubiqPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(DEUBIQUITINATION_VERBS)})\\s+(\\w+)(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    const deubiqResult = extractEntities(deubiqPattern, "deubiquitinates", {
    ...
    const methPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(METHYLATION_VERBS)})\\s+(\\w+)(?:\\s+(?:at|on)\\s+(\\w+))?(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    ...
    // DEMETHYLATION
    const demethPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(DEMETHYLATION_VERBS)})\\s+(\\w+)(?:\\s+(?:at|on)\\s+(\\w+))?(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    ...
    // ACETYLATION
    const acetPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(ACETYLATION_VERBS)})\\s+(\\w+)(?:\\s+(?:at|on)\\s+(\\w+))?(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
    ...
    // DEACETYLATION
    const deacetPattern = new RegExp(
      `^(\\w+)\\s+(?:${buildVerbPattern(DEACETYLATION_VERBS)})\\s+(\\w+)(?:\\s+(?:at|on)\\s+(\\w+))?(?:\\s+(?:with\\s+)?(?:rate|k)\\s+([0-9.e-]+))?`,
      "i",
    );
  ...
  private static parseCompartment(
    text: string,
    match: RegExpMatchArray,
  ): BioSentence {
    const name = match[1];
  ...
  private static parseObservable(
    text: string,
    match: RegExpMatchArray,
  ): ObservableSentence {
